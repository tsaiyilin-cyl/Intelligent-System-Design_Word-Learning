...
- **工作内容:**
  ...
  **修复复习"不认识"未更新熟悉度的问题**：
  - `markUnfamiliar()` 原仅记录复习次数，不更新熟悉度
  - 现在点击"不认识"时熟悉度减15（最低0），与 InteractionService 的 unknown 规则一致

  **移除生词时附带熟悉度提升**：
  - `remove()` 接口新增熟悉度更新：从生词本移除时设为 max(当前, 70)
  - 移除确认对话框增加提示："若移除将认为您已经熟悉该单词"

  **复习提前退出机制**：
  - 复习卡片模式下可以提前退出

  **时间衰减持久化 + 测试答对改为×2**：
  - 之前时间衰减仅在查询时实时计算（applyDecay），存储值不变，导致长期不学后存储值仍虚高
  - 现在每次更新熟悉度前先对存储值施加衰减，再以衰减后的值作为基准计算新值，使衰减效果持久化到数据库
  - 涉及路径：测试答题（InteractionService）、学习模式（WordController/study/updateFamiliarity）、生词本复习（MistakeController）
  - 测试答对与学习模式"熟悉"操作保持统一
...
- **工作内容:**
  ...
  **单词图片服务（Unsplash 免费图库 + 本地缓存）**：
  - 新增 `WordImageService`，实现混合策略：
    1. 优先检查本地缓存 `word-images/{wordId}.jpg`
    2. Unsplash API 搜索单词相关图片（免费，50次/小时）
    3. 下载保存到本地，后续直接读缓存
  - 新增 `WordImageController`，提供 REST API：
    - `GET /api/words/image/{wordId}` — 获取单词图片（不存在则自动生成）
    - `GET /api/words/image/{wordId}/exists` — 检查图片是否存在
    - `POST /api/words/image/preload` — 批量异步预加载
  - 新增 `RestTemplateConfig`，配置 HTTP 客户端超时

  **异步批量预加载，分散生成压力**：
  - 后台线程池异步处理图片生成，不阻塞前端操作
  - 在 6 个单词访问点自动触发预加载：
    - 单词列表加载 → 预加载列表所有单词图片
    - 查看单词详情 → 预加载该单词图片
    - 开始学习模式 → 预加载待学单词
  ...
  **前端学习弹窗展示图片**：
  - 学习卡片（`study-card-big`）在单词释义下方增加图片区域
  - 三种状态：加载中（旋转 spinner + 提示文字）、有图（展示）、无图（占位符）
  - 切换单词时自动触发图片加载
...
- **工作内容:**
  ...
  **自建词表单添加可复制 JSON 示例**：
  ...
  - 新增"填入空模板"功能：插入 `[{"phrase":"","translation":""}]` 空 JSON 结构，用户直接填写内容
  - 新增"复制格式"功能：JOSN 格式代码 `[{"phrase":"...","translation":"..."}]` 以蓝色可点击文本展示，点击即填入带 `...` 占位的模板，替换占位即可使用
  - 每个字段提供三个快捷操作：复制格式 / 填入空模板 / 填入示例数据
